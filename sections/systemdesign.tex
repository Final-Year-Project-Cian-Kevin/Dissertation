...
In this section we will look at some screen shots of the web application from the view of a PC web browser and on a Samsung Galaxy S6 mobile device. For each page we give a brief overview of the features and out summary of the functionality of the page will be provided along with any input validation that was used. As there is alot of pages we will only describe the most important ones. A few screenshots will be provided of the finished site to show the design of the UI.
...
\begin{itemize}
    ...
    \item Positioned under header but sticks to top of page when scrolling down.
    ...
    \item Displays user avatar and username.
    \item Drop down menu contains link to \textit{Profile, Account, Friends, Saved Posts} and an option to logout.
\end{itemize}
...
\begin{itemize}
    ...
    \item Server checks if viewing someone another users profile and displays a follow/unfollow toggle button if it is.
...
\begin{itemize}
    ...
    \item Only values changed will be sent to database to update.
    \item Server validates to ensure email is unique and not already in database returning error message {\color{red}\textit{Email already exists please choose another}} if validation fails.
...
\begin{itemize}
    \item Fully responsive.
    \item Retrieves who a user is following and who is following them from database.
    \item Toggle button to display either followers or following.
    \item Following/ followers are display as card lists with profile avatar, username that acts as a link to their profile and the users bio.
\end{itemize}
